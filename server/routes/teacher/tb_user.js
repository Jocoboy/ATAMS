...
router.get('/', function (req, res, next) {
  switch (req.query.action) {
    case 'mod':
      ...
      pool.query(sql.s_all_f_user_w_account,[req.cookies.teacher2],function(err,modrows){
        if(err){
          console.error(err);
      ...
      break;
    default:
      pool.query(sql.s_all_f_user_w_account,[req.cookies.teacher2], function (err, rows) {
        if (err) {
          console.error(err);
          res.status(500).send({ code: 500, msg: '服务器内部错误！' });
        } else {
